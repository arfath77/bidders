...
class Favourite extends React.Component {
	renderheart = () => {
		let heartFill = "far";
		if (this.props.auth) {
			heartFill = this.props.auth._favourites.find(
				element => element === this.props.id
			)
				? "fas"
				: "far";
	...
	takeAction = () => {
		...
		console.log(id);
		if (this.props.auth) {
			this.props.auth._favourites.find(element => element === id)
		...
		return (
			<>
				<span className="level-item" onClick={this.takeAction}>
					<span className="icon has-text-danger">{this.renderheart()}</span>
				</span>
			</>
		);
	}
