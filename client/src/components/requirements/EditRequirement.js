	...
	renderValues = async () => {
		if (this.props.location.state) {
			return this.setState({
				initialValues: this.props.location.state.element,
			});
		}
	...
	renderContent = () => {
		...
		return (
			<FormTemplate
				...
				onSubmit={formValues =>
					this.props.editRequirement(formValues, this.props.history)
				}
			/>
		);
	};
...
export default connect(mapStateToProps, { fetchOne, editRequirement })(
	withRouter(EditRequirement)
);
